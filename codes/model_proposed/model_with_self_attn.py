...
from keras.layers import Dense, LSTM, GRU, SimpleRNN, Dropout, Conv2D, Lambda, Input, Bidirectional, Flatten
...
def model(x_train, num_labels, LSTM_units, num_conv_filters, batch_size, F, D):
    ...
    rnn_layer = LSTM(LSTM_units, return_sequences=True, name='lstm', return_state=True, implementation=2) # Use implementation=2 for GPU acceleration
...
F = 32
D = 10
DATA_FILES = ['squat_exercise.npz']
MODE = 'LOTO'
BASE_DIR = 'E:/yoga/Yoga-Pose-Classification-and-Skeletonization/encodingHumanActivity/data/LOTO/'
SAVE_DIR = 'E:/yoga/Yoga-Pose-Classification-and-Skeletonization/encodingHumanActivity/model_with_self_attn_LOTO_results'
...
if __name__ == '__main__':
    ...
    for DATA_FILE in DATA_FILES:
        ...
        X = tmp['X']
        print(f'Shape of X before squeeze: {X.shape}')
        ...
        for i in range(0, len(folds)):
            ...
            unique_trainable_weights = {p.ref(): p for p in rnn_model.trainable_weights}
            all_trainable_count = int(np.sum([K.count_params(p) for p in unique_trainable_weights.values()]))
